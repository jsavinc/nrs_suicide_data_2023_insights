...
five_year_averages <- table_3b %>%
  ...
  select(-health_board) %>%
  mutate(sex = factor("Persons",levels = c("Persons","Males","Females")))  # TODO: replace with pivot_longer below once sex is added to data
  # pivot_longer(cols = c(age_standardised_mortality_rate_persons, age_standardised_mortality_rate_males, age_standardised_mortality_rate_females), names_to = "sex", values_to = "asmr", names_transform = ~str_extract(.x, pattern = "(persons|males|females)$")) %>%
  # mutate(sex = factor(sex, levels = c("persons","males","females")))

(fig_five_year_averages <- 
  ggplot(
    data = five_year_averages,
    # TODO: replace with asmr
    aes(x = year_5, y = age_standardised_mortality_rate_persons, colour = sex, group = sex)
  ) +
    geom_line() +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
